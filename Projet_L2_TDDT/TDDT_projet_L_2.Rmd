...
FROM societe s 
  ...
  ON s.siren = ca.siren
WHERE ca.`Chiffres d’affaires nets` != 'NA' 
  AND CAST(ca.`Chiffres d’affaires nets` AS DECIMAL) > 100000000 
...
kaggle_2015 <- ajouter_categorie(kaggle_2015)
kaggle_2016 <- ajouter_categorie(kaggle_2016)
# Fonction pour analyser et visualiser les catégories d'entreprises
analyse_univariee <- function(data, year) {
  # Compter les fréquences
  table_categorie <- table(data$categorie)
  print(table_categorie)
  
  # Calculer les pourcentages
  pourcentage_categorie <- prop.table(table_categorie) * 100
  print(pourcentage_categorie)
  
  # Visualisation : Diagramme en barres
  # print(ggplot(data, aes(x = categorie)) +
  #         geom_bar(fill = "skyblue") +
  #         labs(
  #           title = paste("Répartition des Entreprises par Catégorie (", year, ")", sep = ""),
 ...
 pie(pourcentage_categorie,
    main = paste("Répartition en Pourcentage\ndes Entreprises par Catégorie (", year, ")", sep = ""),
    col = rainbow(length(pourcentage_categorie)))
...
```{r analyse_univariee_2012, eval=TRUE, echo=FALSE, fig.width=6, fig.height=4}

analyse_univariee(kaggle_2012, 2012)
...
\medskip
```{r analyse_univariee_2013, eval=TRUE, echo=FALSE, fig.width=5, fig.height=4}
  analyse_univariee(kaggle_2013, 2013)
```
...
\medskip
```{r analyse_univariee_2014, eval=TRUE, echo=FALSE, fig.width=5, fig.height=4}
  analyse_univariee(kaggle_2014, 2014)
```
...
```{r analyse_univariee_2015, eval=TRUE, echo=FALSE, fig.width=5, fig.height=4}
analyse_univariee(kaggle_2015, 2015)
```
...
```{r analyse_univariee_2016, eval=TRUE, echo=FALSE, fig.width=5, fig.height=4}
analyse_univariee(kaggle_2016, 2016)
```
...
# latex tableur sinon ga qsdqsdqsdqsd
summary_ca_fmt %>%
  kable(format = "latex", booktabs = TRUE, longtable = TRUE,
