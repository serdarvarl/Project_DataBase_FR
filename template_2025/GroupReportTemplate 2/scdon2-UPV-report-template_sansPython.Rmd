...
````{r fig.show='hold', echo=FALSE, warning=FALSE, message=FALSE}
...
fonction_rentabilite <- function(resultat, resultat_fin, ca) {
  if (is.na(resultat) || is.na(resultat_fin) || is.na(ca) || ca == 0) {
    return(NA)
  } else {
    return((resultat + resultat_fin) / ca)
  }
}
...
# 2. Veriyi oku
data_kaggle <- read_csv("data_kaggle.csv")

# 3. 2012-2016 verilerini filtrele ve seç
data_subset <- data_kaggle %>%
  filter(year >= 2012, year <= 2016) %>%
  select(year,
         siren,
         resultat = `Résultat d'exploitation`,
         ca = `Chiffres d’affaires nets`,
         resultat_fin = `Résultat financier`,
         postal_code = postal_code)

# 4. Rentabilité hesapla
data_subset$rentabilite <- apply(data_subset, 1, function(row) {
  resultat <- as.numeric(row["resultat"])
  resultat_fin <- as.numeric(row["resultat_fin"])
  ca <- as.numeric(row["ca"])
  return(fonction_rentabilite(resultat, resultat_fin, ca))
})
...
# 5. Code postal düzelt ve departman çıkar
...
# 6. Veriyi sıraya koy (güzellik için)
data_subset <- data_subset %>%
  select(year, siren, resultat, ca, resultat_fin, code_postal, departement, rentabilite,categorie_rentabilite)

# 7. CSV olarak kaydet
write_csv(data_subset, "resulta_data_kaggle_2012_2016_order.csv")
