...
\tightlist
\item
  \textbf{Profit and loss - Ontology.csv :} Contient les comptes de
  résultat de 100 000 entreprises françaises, avec des informations
  détaillées sur les revenus, les dépenses et les bénéfices. \medskip
\item
  \textbf{APE\_Fusion.csv :} Utilise le code APE pour classer les
  entreprises selon leur secteur d'activité, permettant des comparaisons
  sectorielles précises. \medskip
\item
  \textbf{Data\_Kaggle.csv :} Fournit des données globales sur les
  entreprises, incluant les ventes, les stocks et les taxes, permettant
  d'analyser l'évolution des performances financières sur plusieurs
  années.
\end{itemize}

\medskip

\textbf{Lien vers les données :} Kaggle
Dataset{[}\url{https://www.kaggle.com/datasets/briaclg/financial-data-of-french-compagnies/data?select=Profit+and+loss+-+Onthology.csv}{]}

\section{Descriptif des tables}\label{descriptif-des-tables}

\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1579}}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.0737}}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.5895}}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1789}}@{}}
\caption{Profit and loss - Ontology.csv (nombre de lignes \(\times\)
nombre de colonnes)}\tabularnewline
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Columns (FR/EN) & varchar & Colonnes des états\textbackslash financiers
